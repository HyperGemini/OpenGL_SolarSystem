...
Renderer::Renderer(Display *display, SunShader *sunShader,
                   PlanetShader *planetShader, RegularShader *skyboxShader)
        : display(display), sunShader(sunShader), planetShader(planetShader),
...
void Renderer::render(Entity *entity) {
    ...
    glm::mat4 transform = Math::createTransformationMatrix(
    ...
    planetShader->loadShineVariables(texture.getShineDamper(),
                                     texture.getReflectivity());

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texturedModel.getTexture().getTextureID());
    glDrawElements(GL_TRIANGLES, rawModel.getVertexCount(), GL_UNSIGNED_INT,
                   nullptr);
...
void Renderer::createProjectionMatrix() {
    float aspectRatio = static_cast<float>(display->getWidth()) /
                        static_cast<float>(display->getHeight());
