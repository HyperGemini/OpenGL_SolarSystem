...
RawModel OBJLoader::loadObjModel(std::string const &filename, Loader *loader) {
    ...
    while (std::getline(fin, line)) {
        ...
        if (tokens[0] == "v") {
            positions.emplace_back(stof(tokens[1]), stof(tokens[2]), stof(tokens[3]));
        } else if (tokens[0] == "vt") {
            texCoords.emplace_back(stof(tokens[1]), stof(tokens[2]));
        } else if (tokens[0] == "vn") {
            normals.emplace_back(stof(tokens[1]), stof(tokens[2]), stof(tokens[3]));
        } else if (tokens[0] == "f") {
    ...
    while (std::getline(fin, line)) {
        ...
        processVertex(v1, positions, texCoords, normals, _indices, _positions,
                      _texCoords, _normals);
        processVertex(v2, positions, texCoords, normals, _indices, _positions,
                      _texCoords, _normals);
        processVertex(v3, positions, texCoords, normals, _indices, _positions,
                      _texCoords, _normals);
    }
